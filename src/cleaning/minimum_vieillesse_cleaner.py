...
class MinimumVieillesseCleaner(DatasetCleaner):
    def run(self) -> None:
        self._log("Traitement des données de minimum vieillesse...")
        criteres_data = {}
        ...
        for file in self.input_dir.glob("*.csv"):
            ...
            try:
                ...
                slug = critere.replace(" ", "_").replace("'", "").replace("-", "_")\
        ...
        if not criteres_data:
            self._log("Aucun fichier exploitable pour minimum vieillesse.")
            return
        ...
        self._log(f"minimum_vieillesse_clean.csv généré avec {len(final_df)} lignes et {len(final_df.columns)} colonnes")
