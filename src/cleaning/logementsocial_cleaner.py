...
class LogementSocialCleaner(DatasetCleaner):
    def run(self) -> None:
        self._log("Traitement des données de logements sociaux...")
        ...
        if not dfs:
            self._log("Aucun fichier exploitable pour logements sociaux.")
            return

        final = pd.concat(dfs, ignore_index=True)
        final = final.sort_values(by=["code_departement", "annee"])
        final.to_csv(self.output_file, index=False)
        self._log(f"logements_sociaux_clean.csv généré avec {len(final)} lignes (départements × années).")
