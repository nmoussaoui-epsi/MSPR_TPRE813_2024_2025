...
from src.utils.data_utils import extract_criterion_and_departement, is_value_file, predict_missing_years
from src.utils.constantes import TARGET_YEARS, CLEAN_DIR


class PopulationCleaner(DatasetCleaner):
    def run(self) -> None:
        self._log("Traitement des données de population...")
        criteres_data = {}
        ...
        output_path = CLEAN_DIR / "population_clean.csv"
        final_df.to_csv(output_path, index=False)
        self._log(f"population_clean.csv généré avec {len(final_df)} lignes")
